...
$(function(){
    ...
    //点击搜索按钮时
    $('header .form-inline span').on('click',function(e){
        e.stopPropagation();
    ...
    //点击销量时
     ...
     $('header ul li:nth-child(1)').on('click',function(){
        ...
        //将改变过后的数据写入
        var data5=totalData.slice(0,14);
         $('main ul').html(data5.map(function(item){
                return  `<li class="${item.goodsid}"><div><a href="#">
                <img src="../img/goodsimg/${item.goodsimg}.jpg"></a></div>
                <div><a href="#">${item.goodstitle}</a></div><div>
                <p>￥<span>${item.goodsprice}</span></p>
    ...
    $('main').scroll(function(){
        ...
        if(res<-90){
            num++;
            var data7=totalData.slice(num*14,(num+1)*14);
        $('main ul')[0].innerHTML+=data7.map(function(item){
                return  `<li class="${item.goodsid}"><div><a href="#">
                <img src="../img/goodsimg/${item.goodsimg}.jpg"></a></div>
                <div><a href="#">${item.goodstitle}</a></div><div>
                <p>￥<span>${item.goodsprice}</span></p>
